...
| 页面结构 | 4 个页签：首页 PROFILE / 游戏经历 ARCHIVE / Demo WORKS / 拆解案 TEARDOWN（2026-08-21 起 Demo 提到第三位）；**URL hash 路由**：`#works/loomfall`、`#teardown/economy` 可分享，页签切换进历史、页内状态用 replaceState； |
...
- 游戏经历三档命名：**本命 / 常驻 / 涉猎**（2026-08-21 改，原"挚爱 / 偏爱 / 欣赏"）；档位英文小标只留 `TIER-S/A/B`，不带描述尾巴；大卡内 insight 块叫「思考沉淀 — TAKEAWAYS」，且每卡末尾要有「判断 → 在哪个 Demo/拆解里验证」的钩子
- 拆解案默认显示整体拆解全文，经济/社交/养成三篇用吸顶子页签切换；不放证据徽章和数据来源；每篇开头有「3 分钟速读 — TL;DR」摘要块（问题/核心解法/关键代价/我的结论）；经济篇末尾有「如果让我重新设计」、社交篇末尾有「适用范围与迁移」、养成篇有 FIG.09 玩家决策树
- 5 个 Demo 同页签左右滑动：Loomfall / 龙舟消消乐 / 梗纪元 / GameIntern / 永夜轮回（2026-08-22 重排）；**4 个 AI demo 有"AI 使用与收获"区块（放在制作心得之前），仅梗纪元没有**；玩法介绍统一拆为「玩法简介（一句话核心循环）+ 规则介绍（具体系统）」两块
...
- 文案标点规范：不用「」（直角引号），直接引语用“”；破折号——前后不加空格；`+` 号前后保留空格
